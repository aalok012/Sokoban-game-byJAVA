...
/**
 * Manages the game logic for the 16-Bit Maze game.
 * Tracks player and exit positions, handles movement, and determines win/loss conditions.
 */
public class GameEngine {
	 ...
	 *
	 * @return true if the player's position matches the exit position
	 */
	...
	/**
	 * Checks whether the game is over due to the player running out of steps.
	 *
	 * @return true if the player has no remaining steps
	 */
	...
	/**
	 * Scans the board to locate the exit cell (visible or hidden).
	 * Sets exitRow and exitCol when the EXIT or HIDDEN_EXIT cell is found.
	 */
	...
	/**
	 * Reveals all hidden cells on the board by subtracting the REMOVE_HIDDEN offset.
	 * Called when the player steps onto a MAP cell.
	 */
	private void removeHidden() {
		for (int r = 0; r < 6; r++) {
			for (int c = 0; c < 10; c++) {
				int cell = board.getCell(r, c);
				if (cell >= REMOVE_HIDDEN) {
	 ...
	 * Handles wall collisions, hidden cells, map reveal, first aid kit bonus,
	 * and step counting.
	 *
